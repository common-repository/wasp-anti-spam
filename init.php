...
function ebk_activate() {
  ...
  if (!wp_next_scheduled('ebk_prune_unsent_forms')) {
		wp_schedule_event(time(), 'weekly', 'ebk_prune_unsent_forms');
	}
...
add_filter( 'cron_schedules', 'ebk_add_weekly' ); 
function ebk_add_weekly( $schedules ) {
